...
#      logs/<experiment>/exported_data/<load_run>/model_<checkpoint>.pt
#   2. 调用 play.py 完成回放（视频 + 诊断 CSV 输出到 logs/<experiment>/play_output/）
#   3. 将最新诊断 CSV 打包为 logs/<experiment>/gm_play/model_isaac_csv.pt（SDK PT 上传目录）
...
LEGGED_GYM_ROOT = Path(__file__).resolve().parents[1]  # humanoid/
EXPERIMENT = "x1_dh_stand"
GM_PLAY_DIR = LEGGED_GYM_ROOT / "logs" / EXPERIMENT / "gm_play"
...
def package_csv():
    csvs = sorted(
        glob.glob(str(LEGGED_GYM_ROOT / "logs" / EXPERIMENT / "play_output" / "*isaac_diag.csv"))
    )
...
def main():
    ...
    parser.add_argument("--checkpoint_url_b64", required=True, help="签名 URL 的 URL-safe Base64")
    known, passthrough = parser.parse_known_args()

    url = base64.urlsafe_b64decode(known.checkpoint_url_b64).decode()

    # 从透传参数中提取 load_run / checkpoint，确定下载目标路径
    args_dict = dict(zip(passthrough[::2], passthrough[1::2]))
    load_run = args_dict.get("--load_run", "exported_data")
    checkpoint = args_dict.get("--checkpoint", "model_latest")
    dest = LEGGED_GYM_ROOT / "logs" / EXPERIMENT / "exported_data" / load_run / f"model_{checkpoint}.pt"
    ...
    subprocess.run(cmd, check=True, cwd=str(LEGGED_GYM_ROOT))
